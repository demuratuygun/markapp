...
const LinechartComponent: React.FC<DistributionGraphProps> = ({ 
  ...
  useEffect(() => {
    ...
    let basePoint = Math.floor(Math.min(...data)/25)*25;
    let interval = Math.ceil((Math.max(...data)-basePoint)/25)*25;

    console.log(basePoint, interval);

    setYaxis([basePoint, basePoint+interval]);
    setDatum(data.map((d,i)=>[i+1,d-basePoint]));
  ...
  useEffect(() => {
    ...
    const x = d3.scaleLinear()
      .domain([1, datum.length])
      .range([0, innerSVGwidth]);

    const y = d3.scaleLinear()
      .domain([0, Yaxis[1]-Yaxis[0]])
    ...
    const gback = svg.append('g').attr('transform', `translate(${margin.left},${margin.top})`);
  ...
  return (
    <div style={{ position: "relative", height: "auto" }}>
      ...
      <div style={{ boxShadow: "0px 15px 22px #1d1f1d44", paddingTop: "0px", paddingBottom: "0px", position:"relative",
        ...
        <div className='score-value' style={{ bottom:100, right:11.5 }}> {Yaxis[1]} </div>
        <div className='score-value' style={{ bottom:20, right:11.5 }}> {Yaxis[0]} </div>
